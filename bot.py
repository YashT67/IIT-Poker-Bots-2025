...
RANGE_TIGHT  = 0.25   # Only the top 25% of hands (tight/nit opponent)
RANGE_MEDIUM = 0.50   # Top 50% of hands (average opponent)
RANGE_LOOSE  = 1.00   # All hands (no range constraint applied)
...
REVEALED_HIGH_BLUFF_SCALE =  0.60
...
REVEALED_LOW_EQ_DELTA    = 0.03
REVEALED_LOW_BLUFF_SCALE = 1.20
...
BID_CAP_MAX = 0.60   # Never bid above 64% of pot
...
def evaluate_5(cards: list) -> int:
    """
    Evaluate an exact 5-card hand and return a raw integer strength score.
    Higher score = stronger hand. This is a strict 5-card evaluator.
    """
    e7_cards = convert_to_cards(cards)
    return eval7.evaluate(e7_cards)
...
def preflop_equity(hole: list) -> float:
    ...
    Fallback heuristic (runs only if the key is somehow not in the table,
    which should not happen in practice):
        - Grades the high card on a scale tied to its rank.
        - Pocket pairs receive a double bonus.
        - Suited hands receive +2 to the base score.
        - Connectedness penalty applied proportional to the rank gap.
    ...
    # --- Fallback heuristic (should rarely / never execute) ---

    r1, r2 = card_rank(hole[0]), card_rank(hole[1])
    hi = max(r1, r2)

    # Assign a base quality score to the high card.
    base = {14: 10, 13: 8, 12: 7, 11: 6, 10: 5}.get(hi, hi / 2)

    # Pocket pairs are significantly stronger than unpaired hands.
    if r1 == r2:
        return min(1.0, max(0.0, max(base * 2, 5) / 20.0))

    # Suited hands receive a small bonus due to flush potential.
    if card_suit(hole[0]) == card_suit(hole[1]):
        base += 2

    # Apply a gap penalty: the farther apart the ranks, the less connected
    # the hand is for straight draws. gap=0 → connected (e.g., KQ), gap=3 → 4-gap.
    gap = abs(r1 - r2) - 1
...
#
# This section handles three cases:
#   Case 1: Both opponent hole cards are known (from auction reveal).
#   Case 2: One opponent card is known; the other is sampled from their range.
#   Case 3: Neither opponent card is known; both drawn randomly from range.
# =============================================================================
...
def monte_carlo_equity(hole: list, board: list, opp_known: list,
    ...
    e7_opp_known = [_e7(c) for c in opp_known]
    ...
    for _ in range(n_sims):
        ...
        # ----- CASE 1: Both opponent cards are already known -----
        if cards_needed_opp == 0:
            e7_opp = e7_opp_known
            pool   = remaining

        # ----- CASE 2: One opponent card is known; sample the other -----
        elif range_pairs:
        ...
        # ----- CASE 3: No opponent cards known, unconstrained range -----
        else:
            drawn = _sample(remaining, cards_needed_opp + cards_needed_board)
            opp_hole_strs = list(opp_known) + drawn[:cards_needed_opp]
            e7_opp = [_e7(c) for c in opp_hole_strs]
            e7_bc = [_e7(c) for c in drawn[cards_needed_opp:]]

            my  = _eval(e7_hole + e7_board + e7_bc)
            opp = _eval(e7_opp  + e7_board + e7_bc)
            if my > opp:
                wins += 1.0
            elif my == opp:
                wins += 0.5
...
class BoardTexture:
    ...
    def __init__(self, board: list):
        ...
        # Danger is provided as a semantic alias for wetness.
        # External code can use whichever name is more readable in context.
...
def spr_adjustment(spr: float, equity: float) -> float:
    ...
    if spr < SPR_LOW:
        if equity >= 0.55:
            adj += 0.05    # Already ahead; push toward commitment
        elif equity < 0.40:
            adj -= 0.05    # Behind; discourage calling off chips
...
class OpponentModel:
    ...
    def __init__(self):
        ...
        self.pf_aggressive  = 0   # Pre-flop hands where opponent raised/3-bet

        # Per-street tracking: {street: count of times opponent bet/raised}
        ...
        self.opp_folds   = 0   # Hands opponent folded before showdown (we won early)
        self.showdowns   = 0   # Hands that reached the river

        # Auction-specific counters.
        self.auction_rounds    = 0   # Total auction phases seen
        self.we_won_auction    = 0   # Times our bot won the auction (got a reveal)
        ...
        self.opp_bet_after_auction_count = 0   # Times opp bet flop after winning

        # ---- Exponential Moving Average (EMA) State ----
        # Alpha dictates the decay rate. Equivalent to a ~100 hand window.
        self.ALPHA = 0.05   # ~20-hand effective window (was 0.03 / ~33-hand)
    ...
    def record_hand_end(self, payoff: int, final_street: str, opp_revealed: list):
        ...
        # If the opponent's card(s) were revealed, we must have won the auction.
        if opp_revealed:
    ...
    def vpip_rate(self) -> float:
        """
        Voluntarily Put In Pot rate: fraction of hands where opponent raised/bet
        pre-flop. High VPIP → loose player playing many hands.
        """
    ...
    def fold_rate(self) -> float:
        """
        How often the opponent folds before showdown (across all hands seen).
        High fold rate → they are exploitable by bluffing.
        """
    ...
    def our_auction_win_rate(self) -> float:
        ...
        Starts at 0.5 (neutral assumption) before any auctions are seen.
        """
        return self.we_won_auction / self.auction_rounds if self.auction_rounds > 0 else 0.5
    ...
    def opp_post_auction_bet_rate(self) -> float:
        ...
        otherwise falls back to the session-wide ratio.
        ...
        if self.opp_won_auction_count < 16 or self.ema_opp_auct_bet_rate is None:
            return self.opp_bet_after_auction_count / max(self.opp_won_auction_count, 1)
        return self.ema_opp_auct_bet_rate

    @property
    def recent_vpip(self) -> float:
        """
        Recent pre-flop looseness using only the last _WINDOW hands.
        Falls back to session-wide vpip_rate if insufficient data.
        """
    ...
    def is_tight(self)      -> bool: return self.recent_vpip < 0.25           # Plays <25% of hands
    def is_loose(self)      -> bool: return self.recent_vpip > 0.50           # Plays >50% of hands
    def is_aggressive(self) -> bool: return self.postflop_aggression > 0.45   # Bets >45% of streets
    def is_passive(self)    -> bool: return self.postflop_aggression < 0.25   # Bets <25% of streets
    def is_folder(self)     -> bool: return self.recent_fold_rate > 0.35      # Folds >35% pre-showdown
    ...
    def inferred_range_fraction(self) -> float:
        ...
        Returns:
            RANGE_TIGHT  (0.25) if opponent plays few hands (tight).
            RANGE_LOOSE  (1.00) if opponent plays many hands (loose) OR
...
def decide_action(state: PokerState, equity: float,
    ...
    if we_won_auction and revealed_card and street != 'pre-flop':
        ...
        rev_eq_delta, rev_bluff_sc, rev_size_sc = revealed_card_adjustment(
            revealed_card, board, auctions_won=strategy.model.we_won_auction)
