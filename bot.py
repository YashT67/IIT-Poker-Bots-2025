...
def compute_bid(state: PokerState,
    ...
    min_bid  = BIG_BLIND + SMALL_BLIND
    if strategy.model.our_auction_win_rate < 0.30:
        min_bid += BIG_BLIND

    min_bid = min(min_bid, state.pot)
...
def decide_action(state: PokerState, equity: float,
    ...
    if street_reraise_count > 0:
        value_t = min(0.90, value_t * (1.0 + 0.12 * street_reraise_count))
        call_t = min(0.80, call_t  * (1.0 + 0.07 * street_reraise_count))
...
class Player(BaseBot):
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState) \
        ...
        if self._cached_street == current_state.street and self._cached_equity is not None:
            new_equity = get_equity(current_state, MC_SIMS_AUCTION, opp_rf)
            equity = (self._cached_equity * (1.0 - EQUITY_ALPHA)) + (new_equity * EQUITY_ALPHA)
        else:
            equity = get_equity(current_state, MC_SIMS, opp_rf)
            self._cached_street = current_state.street
